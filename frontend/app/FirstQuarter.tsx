    ...
    (async () => {
      try {
        ...
        if (user) {
          ...
          const userRes = await fetch(`${picked}/users/${user.uid}`);
          if (userRes.ok) {
            const userData = await userRes.json();
            const progressRes = await fetch(`${picked}/progress/${userData.id}`);
            if (progressRes.ok) {
              ...
              currentLessons = currentLessons.map(lesson => {
                ...
                // For now, use simple progress calculation - will be enhanced with activity progress
  ...
  const loadLessonProgress = async () => {
    ...
    try {
      ...
      for (let i = 0; i < updatedLessons.length; i++) {
        ...
        // Priority 1: Calculate ACCURATE progress from activity completion FIRST
        let activityBasedProgress = 0;
        let allDaysCompleted = false;
        
        try {
          // Fetch activity progress for this lesson
          const activityProgress = await getActivityProgress(lesson.id);
          
          // Calculate ACCURATE progress based on actual activity completion
          const lessonContent = LESSON_CONTENT[lesson.id.toString()];
          if (lessonContent && activityProgress.length > 0) {
            let totalActivities = 0;
            let completedActivities = 0;
            
            // Count activities by matching BOTH index AND type to avoid false positives
            lessonContent.days.forEach((day: any, dayIdx: number) => {
              day.activities.forEach((activity: any, actIdx: number) => {
                totalActivities++;
                
                // Check if this specific activity is completed (match index AND type)
                const isCompleted = activityProgress.some((ap: any) => 
                  ap.day_index === dayIdx && 
                  ap.activity_index === actIdx && 
                  ap.activity_type === activity.type
                );
            ...
            activityBasedProgress = totalActivities > 0 
              ? Math.round((completedActivities / totalActivities) * 100) 
              : 0;
            
            // Check if ALL days are 100% complete
            const totalDays = lessonContent.days.length;
            let fullyCompletedDays = 0;
            
            lessonContent.days.forEach((day: any, dayIdx: number) => {
              const dayTotal = day.activities.length;
              const dayCompleted = day.activities.filter((activity: any, actIdx: number) =>
                activityProgress.some((ap: any) => 
                  ap.day_index === dayIdx && 
                  ap.activity_index === actIdx && 
                  ap.activity_type === activity.type
                )
            ...
            allDaysCompleted = (fullyCompletedDays === totalDays);
            
            console.log(`📊 Activity-based progress for lesson ${lesson.id}: ${activityBasedProgress}% (${completedActivities}/${totalActivities} activities, ${fullyCompletedDays}/${totalDays} days complete)`);
          }
        } catch (error: any) {
          // Handle rate limiting specifically
          if (error?.response?.status === 429) {
            console.log(`⏳ Rate limited for lesson ${lesson.id}, using fallback progress`);
          } else {
            console.error(`Error calculating activity progress for lesson ${lesson.id}:`, error);
          }
        }
        
        // Use activity-based progress if available, otherwise fall back to database completion
        if (activityBasedProgress > 0) {
          progressPercentage = activityBasedProgress;
          // Only mark as "completed" if progress is 100% AND all days are fully complete
          status = (progressPercentage >= 100 && allDaysCompleted) ? 'completed' : 'available';
          console.log(`✅ Using activity-based progress for lesson ${lesson.id}: ${progressPercentage}% (status: ${status})`);
        }
        // Priority 2: Check database completion record (only if no activity progress)
        else if (lessonProgress && lessonProgress.completed === 1) {
          progressPercentage = 100;
          status = 'completed';
          console.log(`✅ Lesson ${lesson.id} completed from database: 100%`);
        }
        // Priority 3: Check recent completion data (global state)
        else {
          ...
          // Priority 4: Check database partial progress (fallback)
          else if (progressPercentage === 0 && lessonProgress && lessonProgress.score) {
            progressPercentage = lessonProgress.score;
            status = 'available';
            console.log(`📊 Database progress for lesson ${lesson.id}: ${progressPercentage}%`);
          }
          // Priority 5: Check page-level progress (fallback)
        ...
        // Get lesson pages for additional metadata and update lesson
        try {
