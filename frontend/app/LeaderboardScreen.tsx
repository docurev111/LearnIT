...
// Import border options from ProfileCustomizationModal
const BORDERS = [
  { id: 'none', source: null, name: 'No Border' },
  { id: 'border1', source: require('../assets/images/borders/border1.png'), name: 'Bronze Border' },
  { id: 'border2', source: require('../assets/images/borders/border2.png'), name: 'Silver Border' },
  { id: 'border3', source: require('../assets/images/borders/border3.png'), name: 'Gold Border' },
];
...
export default function LeaderboardScreen() {
  ...
  const fetchLeaderboardData = async () => {
    try {
      ...
      // For weekly tab, sort by actual weekly virtue points
      if (activeTab === 'weekly') {
        leaderboard = leaderboard.map((entry: LeaderboardEntry) => ({
          ...entry,
          virtue_points: entry.weekly_virtue_points || 0, // Use actual weekly virtue points
        })).sort((a: LeaderboardEntry, b: LeaderboardEntry) => (b.virtue_points || 0) - (a.virtue_points || 0));
      }
      // For all-time, backend already sorts by virtue_points
  ...
  // Helper function to get the correct virtue points value based on active tab
  const getVirtuePointsValue = (item: LeaderboardEntry) => {
    return activeTab === 'weekly' ? (item.virtue_points ?? 0) : (item.virtue_points || item.total_xp || 0);
  };
  ...
  const getProfileImage = (user: LeaderboardEntry, position: number) => {
    ...
    const selectedBorder = BORDERS.find(b => b.id === user.selected_border);
    ...
    if (user.profile_picture && user.profile_picture.startsWith('data:image/')) {
      return (
        <View style={leaderboardScreenStyles.profileImageContainer}>
          <Image 
            ...
            resizeMode="cover"
          />
          {selectedBorder && selectedBorder.source && (
            <Image
              source={selectedBorder.source}
              style={leaderboardScreenStyles.profileBorder}
              resizeMode="contain"
            />
          )}
        </View>
  ...
  const renderPodium = () => {
    ...
    // Calculate dynamic heights based on Virtue Points relative to first place
    const maxVirtuePoints = getVirtuePointsValue(first) || 1;
    const baseHeight = 100;
    const minHeight = 60;
    
    const firstHeight = baseHeight;
    const secondHeight = Math.max(minHeight, (getVirtuePointsValue(second) / maxVirtuePoints) * baseHeight * 0.8);
    const thirdHeight = Math.max(minHeight, (getVirtuePointsValue(third) / maxVirtuePoints) * baseHeight * 0.65);
        ...
        <View style={leaderboardScreenStyles.podiumItem}>
          <View style={leaderboardScreenStyles.podiumPlayerInfo}>
            ...
            <View style={leaderboardScreenStyles.xpBadge}>
              <Text style={leaderboardScreenStyles.xpText}>{getVirtuePointsValue(second)} Virtue Points</Text>
            </View>
          </View>
        ...
        <View style={leaderboardScreenStyles.podiumItem}>
          <View style={leaderboardScreenStyles.podiumPlayerInfo}>
            ...
            <View style={[leaderboardScreenStyles.xpBadge, leaderboardScreenStyles.firstPlaceXP]}>
              <Text style={leaderboardScreenStyles.xpText}>{getVirtuePointsValue(first)} Virtue Points</Text>
            </View>
          </View>
        ...
        <View style={leaderboardScreenStyles.podiumItem}>
          <View style={leaderboardScreenStyles.podiumPlayerInfo}>
            ...
            <View style={[leaderboardScreenStyles.xpBadge, leaderboardScreenStyles.thirdPlaceXP]}>
              <Text style={leaderboardScreenStyles.xpText}>{getVirtuePointsValue(third)} Virtue Points</Text>
            </View>
          </View>
            ...
            <View key={item.id} style={[leaderboardScreenStyles.listItem, isCurrentUser && leaderboardScreenStyles.currentUserItem]}>
              ...
              <View style={leaderboardScreenStyles.listPlayerInfo}>
                ...
                <Text style={[leaderboardScreenStyles.listPlayerPoints, isCurrentUser && leaderboardScreenStyles.currentUserSubText]}>
                  {getVirtuePointsValue(item)} {activeTab === 'weekly' ? 'Weekly' : ''} Virtue Points
                </Text>
              </View>
                ...
                <View key={item.id} style={[leaderboardScreenStyles.modalListItem, isCurrentUser && leaderboardScreenStyles.modalCurrentUserItem]}>
                  ...
                  <View style={leaderboardScreenStyles.modalPlayerInfo}>
                    ...
                    <Text style={[leaderboardScreenStyles.modalPlayerPoints, isCurrentUser && leaderboardScreenStyles.modalCurrentUserSubText]}>
                      {getVirtuePointsValue(item)} {activeTab === 'weekly' ? 'Weekly' : ''} Virtue Points • Level {item.current_level || 1}
                    </Text>
                  </View>
                  {position <= 3 && (
